...
def get_dataset(config: Config) -> Dataset:
    ...
    problem_names = ["Problem", "problem", "question", "Question"]
    ...
    if "problem" not in dataset.column_names:
        for name in problem_names:
            if name in dataset.column_names:
                dataset = dataset.rename_column(name, "problem")
                break
    ...
    if "answer" not in dataset.column_names:
        for name in answer_names:
            if name in dataset.column_names:
                dataset = dataset.rename_column(name, "answer")
                break
    
    # Select a subset of the dataset if specified in the config
    if config.dataset_start is not None and config.dataset_end is not None:
        dataset = dataset.select(range(config.dataset_start, config.dataset_end))
    if config.num_samples is not None:
        # dataset = dataset.select(range(min(len(dataset), config.num_samples)))
        num_samples = min(len(dataset), config.num_samples)
        # dataset = dataset.select(range(len(dataset) - num_samples, len(dataset)))
        # dataset = dataset.select([1])
        # dataset = dataset.select([1, 2, 3, 4, 5, 6, 21, 25, 28, 29])
        dataset = dataset.select([1,    2,   3,   4,   5,   6,  18,  21, 25, 26])
...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        if config.output_dir is None:
            config.output_dir = f"data/{config.name}"
