...
def compute_naive_pred(x: Dict[str, List[Any]], n: int) -> Dict[str, str]:
    ...
    paired = sorted(zip(preds, scores), key=lambda ps: ps[1], reverse=True)

    # pick the first non-empty prediction
    for p, s in paired:
        if p:  # non-empty string is truthy
            return {f"pred_naive@{n}": f"\\boxed{{{p}}}"}

    # if all preds were empty, return an empty string (or handle as you prefer)
    return {f"pred_naive@{n}": ""}
...
def find_majority_answer(answers: List[str]) -> str:
    ...
    # Step 1: Group answers by canonical form
    canonical_groups = defaultdict(int)  # Counts per canonical form
    canonical_to_answers = defaultdict(list)  # Answers per canonical form
    for answer in answers:
        cf = memoized_canonical_form(answer)
        canonical_groups[cf] += 1
        canonical_to_answers[cf].append(answer)

    # Step 2: Identify groups with at least one non-empty answer
    valid_groups = {
        cf: count
        for cf, count in canonical_groups.items()
        if any(a != "" for a in canonical_to_answers[cf])
    }

    # Step 3: If no valid groups exist, return ""
    if not valid_groups:
        return ""

    # Step 4: Find the canonical form with the largest count among valid groups
    max_cf = max(valid_groups, key=valid_groups.get)

    # Step 5: Return the first non-empty answer from that group
    for answer in canonical_to_answers[max_cf]:
        if answer != "":
            return answer
    return ""  # Should not reach here due to valid_groups check

def find_answer_with_largest_sum(answers: List[str], scores: List[float]) -> str:
    """
    Groups answers by their canonical forms and selects the group with the largest sum of weighted scores.
    Returns the first non-empty answer from that group, or an empty string if no valid group exists.

    Args:
        answers (List[str]): A list of answer strings.
        scores (List[float]): A list of weights (scores) corresponding to each answer.

    Returns:
        str: The first non-empty answer from the group with the largest weighted sum, or "" if none exist.

    Raises:
        ValueError: If answers or scores lists are empty or of unequal length.
    """
    # Step 1: Validate inputs
    if not answers or not scores or len(answers) != len(scores):
        raise ValueError("answers and scores must be non-empty and of equal length")

    # Step 2: Group answers and sum their weighted scores
    weighted_groups = defaultdict(float)  # Sum of scores per canonical form
    canonical_to_answers = defaultdict(list)  # List of answers per canonical form

    for answer, score in zip(answers, scores):
        canonical_form = memoized_canonical_form(answer)
        weighted_groups[canonical_form] += score
        canonical_to_answers[canonical_form].append(answer)

    # Step 3: Filter for canonical forms with at least one non-empty answer
    valid_cfs = [
        cf for cf in weighted_groups if any(a != "" for a in canonical_to_answers[cf])
    ]

    # Step 4: If no valid groups exist, return an empty string
    if not valid_cfs:
        return ""

    # Step 5: Find the canonical form with the largest weighted sum among valid groups
    max_cf = max(valid_cfs, key=lambda cf: weighted_groups[cf])

    # Step 6: Return the first non-empty answer from the selected group
    return next(a for a in canonical_to_answers[max_cf] if a != "")
