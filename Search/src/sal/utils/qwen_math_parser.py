...
def extract_answer(pred_str, data_name, use_last_number=True):
    # Use the parse function with LaTeX extraction configuration for other datasets
    extraction_config = [
    ...
    if answer_parsed and len(answer_parsed) > 0:
        pred = str(answer_parsed[-1])  # Take the first match as the answer
        # print(answer_parsed, pred)
        return pred
    else:
        # print("KIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIRRRRRRR")
        return ""  # Return empty string if no answer is found


# def extract_answer(pred_str, data_name, use_last_number=True):
#     # Find all \boxed{ans} instances in the text
#     boxed_pattern = r"\\boxed{([^}]*)}"  # Matches \boxed{anything} and captures content inside
#     boxed_matches = re.findall(boxed_pattern, pred_str)

#     # If no boxed answers are found, use the fallback
#     if not boxed_matches:
#         # print(pred_str)
#         print("KIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIRRRRRRR")
#         return ""  # Return empty string if no boxed answers or numbers

#     # Count occurrences of each boxed answer and find the majority
#     answer_counts = Counter(boxed_matches)
#     majority_answer = answer_counts.most_common(1)[0][0]  # Get the most frequent answer

#     # Apply cleaning similar to the original function
#     print(boxed_matches, majority_answer)
